...
function land(){
  clearTimers();const c=encounter,reward=G.addCatch(st,c);save();phase='idle';encounter=null;timing=null;const bonusText=reward.bonus?` Combo ${reward.combo} : +${reward.bonus} ◉ immédiats.`:` Combo ${reward.combo}.`;setFishUi({status:'Prise',dot:'live',headline:'Belle prise.',sub:'Le ferrage était dans la bonne fenêtre.'+bonusText,button:'Relancer',buttonClass:'primary'});$('#result').innerHTML=`<span class="ico">${c.icon}</span><div><b>${c.name}</b><small>${c.rarityLabel} · difficulté ${c.difficulty.toLowerCase()}</small></div><span class="value">${c.value} ◉</span>`;$('#result').classList.remove('hide');if(reward.bonus)toast(`Combo ${reward.combo} · +${reward.bonus} ◉ immédiats`);draw();
}
