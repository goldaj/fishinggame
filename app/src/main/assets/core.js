...
const R={
  commune:{label:'Commune',w:100,g:60,v:18,o:0,wait:[1600,5600],early:[180,650],hit:[1150,1650],late:[300,520],difficulty:'Souple'},
  inhabituelle:{label:'Inhabituelle',w:55,g:28,v:30,o:1,wait:[1500,6000],early:[180,750],hit:[930,1320],late:[290,500],difficulty:'Précise'},
  rare:{label:'Rare',w:25,g:9,v:55,o:2,wait:[1400,6500],early:[170,860],hit:[720,1050],late:[280,470],difficulty:'Délicate'},
  epique:{label:'Épique',w:10,g:2.5,v:100,o:3,wait:[1300,7100],early:[160,980],hit:[550,860],late:[260,440],difficulty:'Exigeante'},
  legendaire:{label:'Légendaire',w:3,g:.45,v:220,o:4,wait:[1200,7800],early:[150,1120],hit:[430,680],late:[240,410],difficulty:'Brève'},
  mythique:{label:'Mythique',w:1,g:.05,v:600,o:5,wait:[1100,8600],early:[140,1280],hit:[320,500],late:[220,380],difficulty:'Fulgurante'}
};
...
function defaultState(){return{version:3,coins:0,unlocked:[1],inventory:{},caughtById:{},totalCaught:0,totalSold:0,totalEarned:0,comboEarned:0,gachaPulls:0,streak:0,maxStreak:0,failedHooks:0,retractedCasts:0,tutorialSeen:false}}
function normalizeState(s){s=Object.assign(defaultState(),s||{});if(!Array.isArray(s.unlocked))s.unlocked=[1];if(!s.unlocked.includes(1))s.unlocked.push(1);s.unlocked=[...new Set(s.unlocked)].filter(id=>id>=1&&id<=100).sort((a,b)=>a-b);s.inventory=s.inventory||{};s.caughtById=s.caughtById||{};s.comboEarned=Math.max(0,Number(s.comboEarned)||0);s.version=3;return s}
...
function fishingTiming(c,rand=Math.random){let p=R[c.rarity];return{waitMs:randBetween(p.wait,rand),earlyMs:randBetween(p.early,rand),strikeMs:randBetween(p.hit,rand),lateMs:randBetween(p.late,rand),difficulty:p.difficulty}}
...
function addCatch(s,c){s.inventory[c.id]=(s.inventory[c.id]||0)+1;s.caughtById[c.id]=(s.caughtById[c.id]||0)+1;s.totalCaught++;s.streak=(s.streak||0)+1;s.maxStreak=Math.max(s.maxStreak||0,s.streak);let bonus=comboBonus(s.streak);if(bonus){s.coins+=bonus;s.totalEarned+=bonus;s.comboEarned=(s.comboEarned||0)+bonus}return{combo:s.streak,bonus}}
...
root.GameCore={rarities:R,creatures,thresholds,rankForSold,nextRankInfo,defaultState,normalizeState,rollCatch,fishingTiming,fishingInputOutcome,comboBonus,addCatch,registerMiss,inventoryValue,inventoryCount,sellAll,eligibleLocked,nextGate,gachaCost,pullGacha,gachaOdds};
