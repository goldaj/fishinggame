...
const positions=['market','fish','collection'].map(x=>navBlock.indexOf(`data-s="${x}"`));
assert.ok(positions.every((x,i)=>x>=0&&(i===0||x>positions[i-1])),'current nav must preserve market, fish, collection order');
assert.ok(!navBlock.includes('grid-template-columns:repeat(4,1fr)'));
assert.ok(!navBlock.includes('data-s="gacha"'),'2.0 removes the gacha navigation entry while keeping the historical v143 module loadable');
...
console.log('v143 compatibility tests passed');
